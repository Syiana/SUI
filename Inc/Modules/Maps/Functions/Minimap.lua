local ADDON, SUI = ...
SUI.MODULES.MINIMAP.Core = function(DB) 
	if (DB and DB.STATE) then
        if not (IsAddOnLoaded("SexyMap")) then
                    ...
                    if not (SUIDB.MINIMAP.OLDSYMBOL) and (garrisonType == LE_GARRISON_TYPE_8_0) then
                        if select(1,UnitFactionGroup("player")) == "Horde" then
                            ...
                            if not gb then
                                ...
                                gb.border:SetAllPoints()
                                gb.border.texture = gb.border:CreateTexture(nil, "OVERLAY")
                                gb.border.texture:SetTexture("Interface\\AddOns\\SUI\\inc\\media\\map\\OrderHallLandingButtonAllianceborder")
            ...
            if DB.CONFIG.HideSymbol == true then
